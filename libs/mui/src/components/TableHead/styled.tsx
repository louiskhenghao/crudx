...
export const StyledTableHead = styled(MuiTableHead, {
  ...
  MuiTableHeadProps &
    Pick<TableHeadProps, 'divider' | 'stickyHeader' | 'tableHeadColumnBorder'> & {
      options?: TableHeadProps['dividerProps'];
    }
>((props) => {
  const { divider, options, theme, stickyHeader, tableHeadColumnBorder } = props;
  const borderStyle = tableHeadColumnBorder === 'preset' ? {
    position: 'relative',
    '::after': {
      content: '""',
      display: 'block',
